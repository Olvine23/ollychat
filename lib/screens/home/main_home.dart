...
class MainHome extends StatelessWidget {
 ...
 Widget _buildPostList(BuildContext context, GetPostState state) {
  if (state.status == GetPostStatus.success) {
    final List<Post> myArticles = state.posts!
        .where((post) => post.myUser.id == user!.uid)
        .toList();
...
Widget _buildArticleSection(BuildContext context, String title, List<Post> articles) {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.stretch,
    children: [
      Padding(
          ...
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => PoemDetailScreen(post: articles[index])),
                );
              },
              child: ArticleCard(
